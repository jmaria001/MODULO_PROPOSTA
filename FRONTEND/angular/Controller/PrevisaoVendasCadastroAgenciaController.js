angular.module('App').controller('PrevisaoVendasCadastroAgenciaController', ['$scope', '$rootScope', 'httpService', '$location', '$timeout', '$routeParams', function ($scope, $rootScope, httpService, $location, $timeout, $routeParams) {
    ...
    $scope.CarregarPrevisao = function (pFiltro) {
        ...
        httpService.Post("CarregarPrevisaoConsisteCompetencia", pFiltro).then(function (responsecheck) {
            if (responsecheck.data) {
                httpService.Post("CarregarPrevisaoCadastroAgencia", pFiltro).then(function (response) {
                    if (response.data) {
    ...
    $scope.CarregarHistorico = function (pPrevisao) {
        ...
        httpService.Post("CarregarHistoricoAgencia", pPrevisao).then(function (response) {
            if (response.data) {
                $scope.Previsao = response.data;
                $scope.AtualizaTotal($scope.Previsao);
            }
    ...
    //    var _anomes = 0;
    //    for (var i = 0; i < pPrevisao.length; i++) {
    //        if (pPrevisao[i].Cod_Agencia && pPrevisao[i].Cod_Cliente) {
    //            for (var x = 1; x < 13; x++) {
    //                _anomes = pPrevisao[i].Ano.toString() + ("0000" + x.toString()).slice(-2);
    //                _valido = false;
    //                if (!pAjuste.Competencia_Inicial && !pAjuste.Competencia_Final) {
    ...
    //                        _novo_Valor = _valor_Previsao + _valor_Ajuste;
    //                    };
    //                    if (x == 1) { pPrevisao[i].Valor_Jan = MoneyFormat(_novo_Valor) };
    //                    if (x == 2) { pPrevisao[i].Valor_Fev = MoneyFormat(_novo_Valor) };
    //                    if (x == 3) { pPrevisao[i].Valor_Mar = MoneyFormat(_novo_Valor) };
    //                    if (x == 4) { pPrevisao[i].Valor_Abr = MoneyFormat(_novo_Valor) };
    //                    if (x == 5) { pPrevisao[i].Valor_Mai = MoneyFormat(_novo_Valor) };
    //                    if (x == 6) { pPrevisao[i].Valor_Jun = MoneyFormat(_novo_Valor) };
    //                    if (x == 7) { pPrevisao[i].Valor_Jul = MoneyFormat(_novo_Valor) };
    //                    if (x == 8) { pPrevisao[i].Valor_Ago = MoneyFormat(_novo_Valor) };
    //                    if (x == 9) { pPrevisao[i].Valor_Set = MoneyFormat(_novo_Valor) };
    //                    if (x == 10) { pPrevisao[i].Valor_Out = MoneyFormat(_novo_Valor) };
    //                    if (x == 11) { pPrevisao[i].Valor_Nov = MoneyFormat(_novo_Valor) };
    //                    if (x == 12) { pPrevisao[i].Valor_Dez = MoneyFormat(_novo_Valor) };
    //                };
    //            };
    //        }
    //    };
    ...
    $scope.PrevisaoExcluirAgenciaCliente = function (pPrevisao) {

        swal({
            title: "Tem certeza que deseja Excluir essa Previsão Agencia/Cliente?",
            type: "warning",
            ...
            httpService.Post('PrevisaoExcluirAgenciaCliente', pPrevisao).then(function (response) {
                if (response) {
                    var _index = -1;
                    if (response.data[0].Status) {
                        for (var i = 0; i < $scope.Previsao.length; i++) {
                            if ($scope.Previsao[i].Cod_Agencia.trim() == pPrevisao.Cod_Agencia.trim() && $scope.Previsao[i].Cod_Cliente.trim() == pPrevisao.Cod_Cliente.trim()) {
                                _index = i;
                                break;
                            }
                        }
    ...
    //==========================Salvar
    $scope.SalvarPrevisaoVendasAgencia = function (pPrevisao, ) {

        httpService.Post("SalvarPrevisaoVendasAgencia", pPrevisao).then(function (response) {
            if (response) {
                if (response.data) {
    ...
    $scope.AdicionarLinha = function (pFiltro,pPrevisao) {
        ...
        httpService.Post('PrevisaVendaNewAgencia', pFiltro).then(function (response) {
            ...
            if (isNaN(pPrevisao[1].Valor_Set)) {
                ...
                $scope.pPrevisao[1].Valor_Set = MoneyFormat(pPrevisao[1].Valor_Set,true);

            }
            else {

                $scope.Previsao[1].Valor_Set = MoneyFormat(pPrevisao[1].Valor_Set,true);
    ...
    //===============Selecionar Agencia
    $scope.SelecionaAgencia = function (pPrevisao) {
        ...
        $scope.PesquisaTabelas.Titulo = "Seleção de Agência";
        $scope.PesquisaTabelas.MultiSelect = false;
        $scope.PesquisaTabelas.ClickCallBack = function (value) { pPrevisao.Cod_Agencia = value.Codigo, pPrevisao.Nome_Agencia = value.Descricao };
        $scope.PesquisaTabelas.LoadCallBack = function (pFilter) {
            httpService.Get('ListarTabela/Terceiro/' + pFilter).then(function (response) {
    ...
    //===============Selecionar Cliente
    $scope.SelecionaCliente = function (pPrevisao) {
        $scope.PesquisaTabelas = NewPesquisaTabela();
        $scope.PesquisaTabelas.Items = [];
        $scope.PesquisaTabelas.PreFiltroTexto = "";
        $scope.PesquisaTabelas.PreFilter = true;
        $scope.PesquisaTabelas.Titulo = "Seleção de Cliente";
        $scope.PesquisaTabelas.MultiSelect = false;
        $scope.PesquisaTabelas.ClickCallBack = function (value) { pPrevisao.Cod_Cliente = value.Codigo, pPrevisao.Nome_Cliente = value.Descricao };
        $scope.PesquisaTabelas.LoadCallBack = function (pFilter) {
            httpService.Get('ListarTabela/Terceiro/' + pFilter).then(function (response) {
                $scope.PesquisaTabelas.Items = response.data;
            });
