angular.module('App').controller('TipoMidiaController', ['$scope', '$rootScope', 'httpService', '$location', '$timeout', function ($scope, $rootScope, httpService, $location, $timeout) {

    //========================Verifica Permissoes
    $scope.PermissaoNew = false;
    $scope.PermissaoEdit = false;
    httpService.Get("credential/TipoMidia@New").then(function (response) {
        $scope.PermissaoNew = response.data;
    });
    httpService.Get("credential/TipoMidia@Edit").then(function (response) {
    ...
    //====================Carrega o Grid
    $scope.CarregarTipoMidia = function () {
        $rootScope.routeloading = true;
        $scope.TipoMidias = [];
        $scope.ShowGrid = false;
        $('#dataTable').dataTable().fnDestroy();
        httpService.Get('TipoMidiaListar').then(function (response) {
            if (response) {
                $scope.TipoMidias = response.data;
                if ($scope.TipoMidias.length === 0) {
    ...
    $scope.$watch('$viewContentLoaded', function () {
        $scope.ConfiguraGrid();
        $scope.CarregarTipoMidia();
    });
    $scope.EditarTipoMidia
