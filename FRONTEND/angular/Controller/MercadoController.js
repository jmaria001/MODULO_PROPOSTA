angular.module('App').controller('MercadoController', ['$scope', '$rootScope', 'httpService', '$location', '$timeout', function ($scope, $rootScope, httpService, $location, $timeout) {

    //========================Verifica Permissoes
    $scope.PermissaoNew = false;
    $scope.PermissaoEdit = false;
    httpService.Get("credential/Mercado@New").then(function (response) {
        $scope.PermissaoNew = response.data;
    });
    httpService.Get("credential/Mercado@Edit").then(function (response) {
    ...
    $scope.gridheaders = [  { 'title': '', 'visible': true, 'searchable': false, 'sortable': false },
                            ...
                            { 'title': 'JOVE', 'visible': true, 'searchable': true, 'sortable': true },
                            { 'title': 'Net', 'visible': true, 'searchable': true, 'sortable': true },
                            ];
    ...
    //====================Carrega o Grid
    $scope.CarregarMercado = function () {
        $rootScope.routeloading = true;
        $scope.Mercados = [];
        $scope.ShowGrid = false;
        $('#dataTable').dataTable().fnDestroy();
        httpService.Get('MercadoListar').then(function (response) {
            if (response) {
                $scope.Mercados = response.data;
                if ($scope.Mercados.length == 0) {
