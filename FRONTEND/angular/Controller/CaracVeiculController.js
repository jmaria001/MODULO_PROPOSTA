angular.module('App').controller('CaracVeiculController', ['$scope', '$rootScope', 'httpService', '$location', '$timeout', function ($scope, $rootScope, httpService, $location, $timeout) {

    //========================Verifica Permissoes
    $scope.PermissaoNew = false;
    $scope.PermissaoEdit = false;
    httpService.Get("credential/CaracVeicul@New").then(function (response) {
        $scope.PermissaoNew = response.data;
    });
    httpService.Get("credential/CaracVeicul@Edit").then(function (response) {
    ...
    $scope.gridheaders = [{ 'title': '', 'visible': true, 'searchable': false, 'sortable': false },
                            ...
                            { 'title': 'Imprime_Ce', 'visible': true, 'searchable': true, 'sortable': true },
                            { 'title': 'Posicao_Calculo', 'visible': true, 'searchable': true, 'sortable': true },
                            { 'title': 'Indica_Basket', 'visible': true, 'searchable': true, 'sortable': true },
    ];
    ...
    //====================Carrega o Grid
    $scope.CarregarCaracVeicul = function () {
        $rootScope.routeloading = true;
        $scope.CaracVeiculs = [];
        $scope.ShowGrid = false;
        $('#dataTable').dataTable().fnDestroy();
        httpService.Get('CaracVeiculListar').then(function (response) {
            if (response) {
                $scope.CaracVeiculs = response.data;
                if ($scope.CaracVeiculs.length == 0) {
