angular.module('App').controller('GeneroController', ['$scope', '$rootScope', 'httpService', '$location', '$timeout', function ($scope, $rootScope, httpService, $location, $timeout) {

    //========================Verifica Permissoes
    $scope.PermissaoNew = false;
    $scope.PermissaoEdit = false;
    httpService.Get("credential/Genero@New").then(function (response) {
        $scope.PermissaoNew = response.data;
    });
    httpService.Get("credential/Genero@Edit").then(function (response) {
    ...
    $scope.CarregarGenero = function () {
        ...
        httpService.Get('GeneroListar').then(function (response) {
            if (response) {
