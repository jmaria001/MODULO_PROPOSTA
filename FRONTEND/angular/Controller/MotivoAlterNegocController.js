angular.module('App').controller('MotivoAlterNegocController', ['$scope', '$rootScope', 'httpService', '$location', '$timeout', function ($scope, $rootScope, httpService, $location, $timeout) {

    //========================Verifica Permissoes
    $scope.PermissaoNew = false;
    $scope.PermissaoEdit = false;
    httpService.Get("credential/MotivoAlterNegoc@New").then(function (response) {
        $scope.PermissaoNew = response.data;
    });
    httpService.Get("credential/MotivoAlterNegoc@Edit").then(function (response) {
    ...
    //====================Carrega o Grid
    $scope.CarregarMotivoAlterNegoc = function () {
        $rootScope.routeloading = true;
        $scope.MotivoAlterNegocs = [];
        $scope.ShowGrid = false;
        $('#dataTable').dataTable().fnDestroy();
        httpService.Get('MotivoAlterNegocListar').then(function (response) {
            if (response) {
                $scope.MotivoAlterNegocs = response.data;
                if ($scope.MotivoAlterNegocs.length == 0) {
    ...
    $scope.$watch('$viewContentLoaded', function () {
        $scope.ConfiguraGrid();
        $scope.CarregarMotivoAlterNegoc();
    });
