angular.module('App').controller('PrevisaoVendasVeiculoController', ['$scope', '$rootScope', 'httpService', '$location', '$timeout', '$routeParams', function ($scope, $rootScope, httpService, $location, $timeout, $routeParams) {
    ...
    //====================Inicializa scopes
    $scope.Previsao = [];
    $scope.gridheaders = [{ 'title': 'Veiculo', 'visible': true, 'searchable': true, 'sortable': true },
    ...
    $scope.CarregarPrevisaoVeiculo = function (pFiltro) {
        console.log('carregando')
        $rootScope.routeloading = true;

        $scope.Previsao = [];
        $('#dataTable').dataTable().fnDestroy();
        httpService.Post('CarregarPrevisaoVendasVeiculo', pFiltro).then(function (response) {
            if (response) {
    ...
    $scope.$watch('$viewContentLoaded', function () {
        ...
        $scope.ConfiguraGrid();
        $scope.CarregarPrevisaoVeiculo($scope.Parameters);
    });
