angular.module('App').controller('CategoriaClienteController', ['$scope', '$rootScope', 'httpService', '$location', '$timeout', function ($scope, $rootScope, httpService, $location, $timeout) {

    //========================Verifica Permissoes
    $scope.PermissaoNew = false;
    $scope.PermissaoEdit = false;
    httpService.Get("credential/CategoriaCliente@New").then(function (response) {
        $scope.PermissaoNew = response.data;
    });
    httpService.Get("credential/CategoriaCliente@Edit").then(function (response) {
    ...
    $scope.gridheaders = [{ 'title': '', 'visible': true, 'searchable': false, 'sortable': false },
                            ...
                            { 'title': 'Cod_Fiscal', 'visible': true, 'searchable': true, 'sortable': true },
    ];
    ...
    //====================Carrega o Grid
    $scope.CarregarCategoriaCliente = function () {
        $rootScope.routeloading = true;
        $scope.CategoriaClientes = [];
        $scope.ShowGrid = false;
        $('#dataTable').dataTable().fnDestroy();
        httpService.Get('CategoriaClienteListar').then(function (response) {
            if (response) {
                $scope.CategoriaClientes = response.data;
                if ($scope.CategoriaClientes.length == 0) {
    ...
    $scope.$watch('$viewContentLoaded', function () {
        $scope.ConfiguraGrid();
        $scope.CarregarCategoriaCliente();
    });
