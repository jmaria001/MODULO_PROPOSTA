angular.module('App').controller('CondPgtoController', ['$scope', '$rootScope', 'httpService', '$location', '$timeout', function ($scope, $rootScope, httpService, $location, $timeout) {
    //========================Verifica Permissoes
    $scope.PermissaoNew = false;
    $scope.PermissaoEdit = false;
    httpService.Get("credential/CondPgto@New").then(function (response) {
        $scope.PermissaoNew = response.data;
    });
    httpService.Get("credential/CondPgto@Edit").then(function (response) {
    ...
    { 'title': 'Qtd_Dias', 'visible': true, 'searchable': true, 'sortable': true },
    { 'title': 'Tipo_Vencimento', 'visible': true, 'searchable': true, 'sortable': true },
    ];
    ...
    //====================Carrega o Grid
    $scope.CarregarCondPgto = function () {
        $rootScope.routeloading = true;
        $scope.CondPgtos = [];
        $scope.ShowGrid = false;
        $('#dataTable').dataTable().fnDestroy();
        httpService.Get('CondPgtoListar').then(function (response) {
            if (response) {
                $scope.CondPgtos = response.data;
                if ($scope.CondPgtos.length == 0) {
    ...
    $scope.$watch('$viewContentLoaded', function () {
        $scope.ConfiguraGrid();
        $scope.CarregarCondPgto();
    });
    $scope.EditarCondPgto
