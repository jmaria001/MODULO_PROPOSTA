...
namespace PROPOSTA
{
    public class ImpressaoMapa
    {
        ...
        public ImpressaoMapa(String pUser)
        {
            this.Credential = pUser;
            this.CurrentUser = clsLib.Decriptografa(clsLib.GetJsonItem(this.Credential, "Name"));
        }
        public String ImprimirMapa(Int32 pId_Contrato)
        {
            ...
            try
            {
                //==================Verifica se existe o Mapa Reserva e se Pertence ao usuario
                ...
                if (true)
                {
                    ...
                    if (GerarPdf(pId_Contrato, sPath, strFilePdf))
                    {
                        PdfFinal = strFilePdf;
                    }
                }
        ...
        private Boolean GerarPdf(Int32 pId_Contrato, String Path, String FileName)
        {
            ...
            try
            {
                ...
                DataTable dtbMapa = new DataTable("Contrato");
                SqlDataAdapter dtaMapa = new SqlDataAdapter();
                
                cnn.Open();
                dtaMapa.SelectCommand = cnn.Text(cnn.Connection, "Select Distinct Id_Contrato From Contrato with (Nolock) Where Id_Contrato = " + pId_Contrato);
                dtaMapa.Fill(dtbMapa);

                foreach (DataRow drw in dtbMapa.Rows)
                {
                    this.ImprimeCabecalho(write, doc, drw["Id_Contrato"].ToString().ConvertToInt32());
                    this.ImprimDadosBase(write, doc, drw["Id_Contrato"].ToString().ConvertToInt32());
                    this.ImprimeComerciais(write, doc, drw["Id_Contrato"].ToString().ConvertToInt32());
                    this.ImprimeInsercao(write, doc, drw["Id_Contrato"].ToString().ConvertToInt32());
                    this.ImprimeEmissoras(write, doc, drw["Id_Contrato"].ToString().ConvertToInt32());
                }
        ...
        private void ImprimDadosBase(PdfWriter ww, Document dd, Int32 pId_Contrato)
        {
            try
            {
                //==================Carrega Dados Base
                ...
                clsConexao cnn = new clsConexao(this.Credential);
                cnn.Open();
                dta.SelectCommand = cnn.Procedure(cnn.Connection, "Pr_Proposta_ImprimeMr_DadosBase");
                dta.SelectCommand.Parameters.AddWithValue("@Par_Login", this.CurrentUser);
                dta.SelectCommand.Parameters.AddWithValue("@Par_Id_Contrato", pId_Contrato);
                dta.Fill(dtb);
        ...
        private void ImprimeCabecalho(PdfWriter ww, Document dd, Int32 pId_Contrato)
        {
            PageNumer++;
            if (PageNumer > 1)
            {
            ...
            clsPdf.addCell(tbDados, new pdfLibCell() { Text = "N.Contrato" });
            clsPdf.addCell(tbDados, new pdfLibCell() { Text = drw["Contrato"].ToString() });
            clsPdf.addCell(tbDados, new pdfLibCell() { Text = "Data Recepção" });
            clsPdf.addCell(tbDados, new pdfLibCell() { Text = drw["Data_Recepcao"].ToString() });
        ...
        private void ImprimeComerciais(PdfWriter ww, Document dd, Int32 pId_Contrato)
        {
            PdfContentByte pc;
            ...
            clsConexao cnn = new clsConexao(this.Credential);
            cnn.Open();
            dta.SelectCommand = cnn.Procedure(cnn.Connection, "Pr_Proposta_ImprimeMr_Comercial");
            dta.SelectCommand.Parameters.AddWithValue("@Par_Login", this.CurrentUser);
            dta.SelectCommand.Parameters.AddWithValue("@Par_Id_Contrato", pId_Contrato);
            dta.Fill(dtb);
            //==================Imprime
            ...
            foreach (DataRow drwC in dtb.Rows)
            {
                if (CurrentPosition - tbComercial.TotalHeight < 50)
                {
                    ...
                    this.ImprimeCabecalho(ww, dd, pId_Contrato);
                }
                if (BolPrimeiro)
                {
                ...
                clsPdf.addCell(tbComercial, new pdfLibCell() { Text = drwC["Titulo_Comercial"].ToString(), Align = PdfPCell.ALIGN_LEFT  });
                clsPdf.addCell(tbComercial, new pdfLibCell() { Text = drwC["Duracao"].ToString() });
                clsPdf.addCell(tbComercial, new pdfLibCell() { Text = drwC["Nome_Tipo_Comercial"].ToString(), Align = PdfPCell.ALIGN_LEFT });
        ...
        private void ImprimeInsercao(PdfWriter ww, Document dd, Int32 pId_Contrato)
        {
            PdfContentByte pc;
            Boolean hasRow = false;
            Boolean BolPrimeiro;
            ...
            clsConexao cnn = new clsConexao(this.Credential);
            cnn.Open();
            dta.SelectCommand = cnn.Procedure(cnn.Connection, "Pr_Proposta_ImprimeMr_Midia");
            dta.SelectCommand.Parameters.AddWithValue("@Par_Login", this.CurrentUser);
            dta.SelectCommand.Parameters.AddWithValue("@Par_Id_Contrato", pId_Contrato);
            dta.Fill(dtb);
            //==================Imprime
            ...
            Grid.Add(new TableTemplate() { Size = 30, Header = "Car.", Field = "Cod_Caracteristica" });
            Grid.Add(new TableTemplate() { Size = 30, Header = "Leg.", Field = "Cod_Comercial" });
            ...
            Grid.Add(new TableTemplate() { Size = 80, Header = "Valor Tabela", Field = "Valor_Tabela", Align = PdfPCell.ALIGN_RIGHT });
            Grid.Add(new TableTemplate() { Size = 80, Header = "Valor Negociado", Field = "Valor_Negociado", Align = PdfPCell.ALIGN_RIGHT });
        ...
        private void ImprimeEmissoras(PdfWriter ww, Document dd, Int32 pId_Contrato)
        {
            PdfContentByte pc;
            Boolean hasRow = false;
            Boolean BolPrimeiro;
            ...
            clsConexao cnn = new clsConexao(this.Credential);
            cnn.Open();
            dta.SelectCommand = cnn.Procedure(cnn.Connection, "Pr_Proposta_ImprimeMR_Emissoras");
            dta.SelectCommand.Parameters.AddWithValue("@Par_Login", this.CurrentUser);
            dta.SelectCommand.Parameters.AddWithValue("@Par_Id_Contrato", pId_Contrato);
            dta.Fill(dtb);
